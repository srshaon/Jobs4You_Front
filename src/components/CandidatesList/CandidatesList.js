...
const CandidatesList = () => {
  ...
  <tbody>
   {candidates?.slice(visitedPage,visitedPage+candidatePerPage).map(({_id,fname,pEmail,pContact},index)=><tr>
      <td>{index+1}</td>
      <td><img className='custom-img me-2' src='https://th.bing.com/th/id/OIP.c2GTibztP9NW0_ITWQ2qwwHaGo?pid=ImgDet&rs=1'alt="" /> {fname}</td>
      <td>{pEmail}</td>
      <td>{pContact}</td>
      <td> <Link to={`/candidates/${_id}`}><span><AiFillEye/></span></Link></td>
      <td> <Link to={`/profileedit/${_id}`}><span><AiTwotoneEdit/></span></Link></td>
    </tr>)}
  </tbody>
</Table>
